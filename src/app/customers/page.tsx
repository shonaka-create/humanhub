...
import { customerCount, customerDetail, customerList } from '@/lib/mock';
...
export default function CustomersPage() {
  ...
  if (view === 'bulk') return <BulkView onBack={() => setView('detail')} />;
  return <DetailView onBulk={() => setView('bulk')} />;
}

/* ---------------- Detail (list + selected customer) ---------------- */
function DetailView({ onBulk }: { onBulk: () => void }) {
  const { t } = useLang();
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
        ...
        <button style={{ marginLeft: 'auto', font: '500 12.5px var(--ui)', border: '1px solid var(--line)', background: '#fff', borderRadius: 999, padding: '9px 16px', cursor: 'pointer', color: 'var(--ink)', whiteSpace: 'nowrap' }}>＋ {t.btnAddCustomer}</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '318px 1fr', gap: 22, alignItems: 'start' }}>
        ...
        <div style={{ background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 16, padding: 18, boxShadow: '0 1px 2px rgba(46,42,37,.04)' }}>
          ...
          {customerList.map((c) => (
            <div
              key={c.id}
              className={c.active ? undefined : 'row-hover'}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: 11,
                ...
                cursor: 'pointer',
                marginTop: 8,
                background: c.active ? 'var(--accent-soft)' : undefined,
              }}
            >
              <Avatar initial={c.initial} tone={c.tone} size={38} solid={c.tone === 'accent' && c.active} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.25fr 1fr', gap: 22, alignItems: 'start' }}>
            ...
            <div style={{ background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 16, padding: '22px 24px', boxShadow: '0 1px 2px rgba(46,42,37,.04)' }}>
              ...
              <div style={{ marginTop: 14, borderTop: '1px solid var(--line)', paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 9 }}>
                <textarea placeholder={t.memoAddPlaceholder} style={{ border: '1px solid var(--line)', borderRadius: 12, padding: '11px 13px', font: '12.5px var(--ui)', outline: 'none', background: '#FBF9F5', resize: 'none', height: 52, width: '100%', boxSizing: 'border-box', color: 'var(--ink)' }} />
                <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                  <select style={{ border: '1px solid var(--line)', borderRadius: 999, padding: '8px 13px', font: '12px var(--ui)', background: '#fff', color: 'var(--ink2)', cursor: 'pointer' }}>
                    <option>Emma</option><option>Aoi</option><option>Mei</option><option>Yuki</option>
                  </select>
                  <button style={{ marginLeft: 'auto', font: '600 12px var(--ui)', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 999, padding: '9px 20px', cursor: 'pointer' }}>{t.memoAdd}</button>
                </div>
              </div>
            </div>
            ...
            <div style={{ background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 16, padding: '22px 24px', boxShadow: '0 1px 2px rgba(46,42,37,.04)' }}>
              ...
              <div style={{ position: 'relative', paddingLeft: 26 }}>
                ...
                <div style={{ position: 'relative', marginBottom: 18 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--ink2)' }}>6/24 11:30</div>
                </div>
                <div style={{ position: 'relative', marginBottom: 18 }}>
...
/* ---------------- Bulk email compose ---------------- */
function BulkView({ onBack }: { onBack: () => void }) {
  const { t } = useLang();
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 22, alignItems: 'start' }}>
        <div style={{ background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 16, padding: '24px 26px', boxShadow: '0 1px 2px rgba(46,42,37,.04)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 18, borderTop: '1px solid var(--line)', paddingTop: 16 }}>
            ...
            <button style={{ font: '600 12px var(--ui)', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 999, padding: '9px 18px', cursor: 'pointer', whiteSpace: 'nowrap' }}>{t.bulkSend} · 18</button>
          </div>
        </div>
        ...
        <div style={{ background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 16, padding: '22px 24px', boxShadow: '0 1px 2px rgba(46,42,37,.04)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 5, paddingBottom: 14, borderBottom: '1px solid var(--line)', marginBottom: 14 }}>
            <span style={{ fontFamily: 'var(--serif)', fontSize: 38, fontWeight: 600, lineHeight: 0.9 }}>18</span>
          ...
          <div style={{ fontSize: 11.5, color: 'var(--ink3)', padding: '8px 0 0' }}>他 14名…</div>
        </div>
      </div>
    </>
  );
